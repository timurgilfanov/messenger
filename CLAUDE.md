...
- **Backend Chat Contract**: For every chat participant list returned by the backend, there must be exactly one participant with `isCurrentUser=true`; treat zero or multiple current-user participants as an invalid payload
...
- **MVI Pattern**: ViewModels expose a single read-only `StateFlow` for persistent UI state and a `Channel`-backed `Flow` for one-off side effects (navigation, toasts, input clearing)
  ...
  - Cross-cutting navigation side effects (e.g., session expiry navigating to Login) are handled centrally in `MainActivityViewModel`, not propagated as `onAuthFailure`-style callbacks through the screen hierarchy. `MainActivityViewModel` observes `authState` continuously: the first emission sets the initial destination (guarded by `_uiState.value is Loading`), and any subsequent `Unauthenticated` emission sends a `NavigateToLogin` effect. `MainActivity` collects this effect via `repeatOnLifecycle(STARTED)` and clears the back stack to the Login destination. Individual screens must not introduce per-screen auth-failure callbacks.
...
- After changes that may affect compilation (e.g. modifying interfaces, adding/removing methods, changing signatures): run `./gradlew :app:compileMockDebugAndroidTestKotlin` to verify the androidTest sources compile.
- Do not modify static analysis configuration files (e.g. `detekt*.yml`, `.editorconfig`, `ktlint` config) or suppress compilation warnings with `@Suppress` without explicit confirmation from the user.
