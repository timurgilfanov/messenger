...
import timur.gilfanov.messenger.data.source.remote.DeltaOperation
    ...
    override suspend fun applyChatDelta(
        ...
        when (delta.operation) {
            DeltaOperation.CREATE -> {
                val chatMetadata = delta.chatMetadata!!
                val newChat = Chat(
                    id = delta.chatId,
                    participants = chatMetadata.participants,
                    name = chatMetadata.name,
                    pictureUrl = chatMetadata.pictureUrl,
                    rules = chatMetadata.rules,
                    unreadMessagesCount = chatMetadata.unreadMessagesCount,
                    lastReadMessageId = chatMetadata.lastReadMessageId,
                    messages = delta.incrementalMessages,
                )
            ...
            DeltaOperation.UPDATE -> {
                ...
                val messages = updateMessages(chat, delta)
