...
class SettingsRepositoryImplTest {
    ...
    private lateinit var remoteDataSourceStub: RemoteSettingsDataSourceStub
    ...
    fun setup() {
        ...
        remoteDataSourceStub = RemoteSettingsDataSourceStub().apply {
            setGetResponse(
        ...
        repository = SettingsRepositoryImpl(
            ...
            remoteDataSource = remoteDataSourceStub,
    ...
    private fun useRemoteDataSourceFake() {
        repository = SettingsRepositoryImpl(
            localDataSource = localDataSource,
    ...
    fun `observeSettings returns default settings when no entities exist`() = runTest {
        repository.observeSettings(session).test {
    ...
    fun `changeUiLanguage updates existing entity`() = runTest {
        ...
        val result = repository.changeUiLanguage(session, UiLanguage.German)
    ...
    fun `syncSetting returns Success when already synced`() = runTest {
        ...
        val outcome = repository.syncSetting(session, SettingKey.UI_LANGUAGE)
    ...
    fun `syncSetting maps local get errors`() = runTest {
        ...
        val cases = listOf(
            ...
            GetSettingError.StorageFull to SyncSettingRepositoryError.LocalOperationFailed(
        ...
        cases.forEach { (source, expected) ->
            ...
            val result = repository.syncSetting(session, SettingKey.UI_LANGUAGE)

            assertIs<ResultWithError.Failure<Unit, SyncSettingRepositoryError>>(result)
            assertEquals(expected::class, result.error::class)
            if (expected is SyncSettingRepositoryError.LocalOperationFailed &&
                expected.error is LocalStorageError.UnknownError
            ) {
                val localOpFailed = result.error as SyncSettingRepositoryError.LocalOperationFailed
                val unknownError = localOpFailed.error as LocalStorageError.UnknownError
    ...
    fun `syncAllPendingSettings returns Success when no unsynced settings`() = runTest {
        val outcome = repository.syncAllPendingSettings(session)
    ...
    fun `syncAllPendingSettings syncs multiple settings`() = runTest {
        ...
        remoteDataSourceFake.setSyncBehavior { request ->
            ResultWithError.Success(
                SyncResult.Success(
                    newVersion = request.request.clientVersion + 1,
                ),
            )
        }
    ...
    fun `syncAllPendingSettings returns RemoteSyncFailed when sync fails`() = runTest {
        ...
        val result = repository.syncAllPendingSettings(session)
    ...
    fun `observeSettings triggers recovery with valid remote settings`() = runTest {
        ...
        repoWithRemote.observeSettings(session).test {
            val result = awaitItem()
            assertIs<ResultWithError.Success<Settings, GetSettingsRepositoryError>>(result)
            assertEquals(UiLanguage.German, result.data.uiLanguage)
    ...
    fun `observeSettings maps local errors`() = runTest {
        ...
        val cases = listOf(
            ...
            GetSettingsLocalDataSourceError.UnknownError(cause) to
                LocalStorageError.UnknownError(cause),
        )

        cases.forEach { (sourceError, expectedLocalError) ->
            ...
            repo.observeSettings(session).test {
                val result = awaitItem()
                assertIs<ResultWithError.Failure<Settings, GetSettingsRepositoryError>>(result)
                val error = result.error
                assertIs<GetSettingsRepositoryError.LocalOperationFailed>(error)
                assertEquals(expectedLocalError::class, error.error::class)
                if (expectedLocalError is LocalStorageError.UnknownError) {
                    assertEquals(
                        cause,
    ...
    fun `changeUiLanguage with no local settings triggers recovery then applies change`() =
        runTest {
            ...
            val result = repoWithRemote.changeUiLanguage(session, UiLanguage.German)
    ...
    fun `changeUiLanguage maps transform errors`() = runTest {
        ...
        val cases = listOf(
            TransformSettingError.AccessDenied to LocalStorageError.AccessDenied,
            TransformSettingError.DiskIOError to LocalStorageError.TemporarilyUnavailable,
            TransformSettingError.ConcurrentModificationError to
                LocalStorageError.TemporarilyUnavailable,
            TransformSettingError.DatabaseCorrupted to LocalStorageError.Corrupted,
            TransformSettingError.ReadOnlyDatabase to LocalStorageError.ReadOnly,
            TransformSettingError.StorageFull to LocalStorageError.StorageFull,
            TransformSettingError.UnknownError(cause) to LocalStorageError.UnknownError(cause),
        )

        cases.forEach { (transformError, expectedLocalError) ->
    ...
    fun `changeUiLanguage returns UnknownError when recovery upsert fails unexpectedly`() =
        runTest {
            ...
            val result = repoWithRemote.changeUiLanguage(session, UiLanguage.English)
    ...
    @Test
    fun `changeUiLanguage returns StorageFull when recovery defaults fail to upsert`() = runTest {
        val stub = RemoteSettingsDataSourceStub()
        stub.setGetResponse(
            ResultWithError.Failure(
                RemoteSettingsDataSourceError.RemoteDataSource(
    ...
    fun `observeSettings triggers recovery with invalid server value falls back to English`() =
        runTest {
            val remoteWithInvalidValue = object : RemoteSettingsDataSource {
                override suspend fun get(): ResultWithError<
                    RemoteSettings,
                    RemoteSettingsDataSourceError,
                    > {
            ...
            repoWithInvalidRemote.observeSettings(session).test {
                val result = awaitItem()
                assertIs<ResultWithError.Success<Settings, GetSettingsRepositoryError>>(result)
                assertEquals(
                    UiLanguage.English,
    ...
    fun `changeUiLanguage with invalid remote value recovers with fallback then applies change`() =
        runTest {
            ...
            val result = repoWithInvalidRemote.changeUiLanguage(session, UiLanguage.German)
