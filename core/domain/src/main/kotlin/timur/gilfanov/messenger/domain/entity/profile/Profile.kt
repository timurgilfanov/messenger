package timur.gilfanov.messenger.domain.entity.profile

/**
 * User profile information.
 *
 * Represents publicly visible user data that can be updated by the user.
 *
 * @property name Display name (validated via [timur.gilfanov.messenger.domain.usecase.profile.UpdateNameError])
 * @property pictureUrl URL to user's profile picture, null if no picture set
 */
data class Profile(val name: String, val pictureUrl: String?) {
    override fun toString(): String = name
}
